...
The loss function measures the performance on a single batch and its result is what the network will minimize during the backpropagation. Common choices include the mean squared error (MSE) for regression problems or the categorical cross entropy (CCE) for classification problems. 
...
Both describe the overlap of ground truth and prediction in relation to falsely identified segments. Higher scores of these measurements indicate a better segmentation. Since the backpropagation will minimize the result of the loss function, the negative DICE coefficient was used for this study.
