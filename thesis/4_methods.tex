...
Common batch sizes range from 32 to 256, but depend on the size of the input and the available RAM of the workstation. In the field of 3D convolutions In the past it was believed that larger batches led to something called the generalization gap (1609.04836), where the accuracy of a model would drop if it was trained particularly large batches. Recent work by Hoffer at al. (1705.08741) suggest other reasons for this drop in accuracy.

For 2D CNNs batch sizes between 32 and 256 are common, but recent experiments by Goyal et al. show

The batch size describes the number of samples that will be used for one forward- and back-propagation step. In the past it was believed that larger batches led to a phenomenon called the generalization gap.
