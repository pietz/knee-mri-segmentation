...
The worst results appeared in upper and lower slices where the bones started to become visible. Oftentimes the network didn't agree with the ground truth on what could already be interpreted as bone. If either the prediction or the ground truth was empty while the other was not, then the result is a DSC score of 0\%.

\subsection{Model Visualization}

Neural networks are often considered black boxes because it is difficult to get an intuitive understanding how decisions are made. This is a crucial problem especially in the medical field, where a prediction makes estimates about the health of a patient.

Two factors help in this study to pull away from this problem. Firstly, a segmentation is an image to image pipeline making it less abstract what kind of calculations are happening. Visualizing input and output gives an intuitive understanding of the transformation the network applied from one to the other.
